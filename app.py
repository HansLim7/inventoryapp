...
def update_quantity(conn, product, size, new_quantity):
    try:
        existing_data = load_data(conn)
        mask = (existing_data['PRODUCT'] == product) & (existing_data['SIZE'] == size)
        ...
        st.success(f"Inventory updated for {product} (Size: {size}). New quantity: {new_quantity}")
    except Exception as e:
        ...
        with st.sidebar:
            ...
            if st.button("Update Inventory"):
                if quantity > 0:
                    ...
                    update_quantity(conn, selected_product_to_update, selected_size_to_update, new_quantity)
                else:
